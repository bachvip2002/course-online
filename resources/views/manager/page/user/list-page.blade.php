    ...
    <div id="kt_app_toolbar" class="app-toolbar py-3 py-lg-6">
        ...
        <div id="kt_app_toolbar_container" class="app-container container-xxl d-flex  flex-wrap flex-stack">
            ...
            <div class="d-flex align-items-center gap-2 gap-lg-3">
                <a href="{{ route('manager.user.create-page') }}" class="btn btn-sm btn-success">
...
@section('kt_app_content')
    ...
    <div class="card card-flush">
        <form class="" action="{{ route('manager.user.list-page') }}">
            @method('GET')
            <!--begin::Card header-->
            <div class="card-header align-items-center justify-content-start gap-md-5">
                ...
                <div class="d-flex align-items-center position-relative my-1">
                    <i class="ki-outline ki-magnifier fs-3 position-absolute ms-4"></i>
                    <input type="text" value="{{ request()->query('phone_number') }}" name="phone_number"
                        data-kt-ecommerce-product-filter="search" class="form-control w-250px ps-12"
                ...
                <div class="d-flex align-items-center position-relative my-1">
                    ...
                    <select class="form-select w-200px" name="status" data-control="select2" data-hide-search="true"
                        ...
                        @foreach ($defaultStatues as $statusId => $status)
                            <option {{ request()->query('status') == $statusId ? 'selected' : '' }}
                                value="{{ $statusId }}">
                                {{ $status }}
                            </option>
                        @endforeach
                    ...
                    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_user_table">
                        <thead>
                            <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                                ...
                                <th class="min-w-200px">Họ và tên</th>
                                <th class="text-end min-w-100px">Số điện thoại</th>
                                <th class="text-end min-w-70px">Địa chỉ</th>
                                <th class="text-end min-w-100px">ID đăng nhập</th>
